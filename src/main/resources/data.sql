...
((30.0),
 ...
 (SELECT id from  seller where user_id IN ( SELECT id FROM user where name='Romaria')));

INSERT INTO advertisement ( price, title, product_id, seller_id) VALUES
((30.0),
 ('Condicionador mais top'),
 (SELECT id from product where name='Condicionador'),
 (SELECT id from  seller where user_id IN ( SELECT id FROM user where name='Maria')));
